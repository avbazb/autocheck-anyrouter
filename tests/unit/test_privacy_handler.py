...
class TestPrivacyHandler:
	...
	@pytest.mark.parametrize(
		'show_sensitive,account,expected_safe_name,should_mask',
		[
			# 默认行为：仓库视为私有，不脱敏
			(None, {'name': '我的账号', 'cookies': 'test', 'api_user': 'user1'}, '我的账号', False),
			# 默认行为：默认名称（不会被脱敏）
			(None, {'cookies': 'test', 'api_user': 'user2'}, '账号 2', False),
			# 显式禁用敏感信息（会被脱敏）
			('false', {'name': '我的账号', 'cookies': 'test', 'api_user': 'user1'}, '我', True),
			# 强制显示敏感信息（不脱敏）
			('true', {'name': '我的账号', 'cookies': 'test', 'api_user': 'user1'}, '我的账号', False),
			# 显式禁用：Emoji 账号名称
			('false', {'name': '😀测试账号', 'cookies': 'test', 'api_user': 'user1'}, '😀', True),
			# 显式禁用：超长账号名称（测试边界）
			(
				'false',
	...
	@pytest.mark.parametrize(
		'show_sensitive,quota,used,expected_has_numbers',
		[
			# 默认行为：仓库视为私有，显示余额
			(None, 50.0, 10.0, True),
			# 显式禁用敏感信息（隐藏余额）
			('false', 50.0, 10.0, False),
			# 强制显示
			('true', 50.0, 10.0, True),
			# 大数字边界测试
			(None, 999999.99, 888888.88, True),
			# 零值测试
			(None, 0.0, 0.0, True),
		],
	)
